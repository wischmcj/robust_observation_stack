...
do
  docker run -d --name archiveteam_$i --publish $port:8001 --label=com.centurylinklabs.watchtower.enable=true --log-driver json-file --log-opt max-size=50m --restart=on-failure  atdr.meo.ws/archiveteam/usgovernment-grab --concurrent 6 penguaman
